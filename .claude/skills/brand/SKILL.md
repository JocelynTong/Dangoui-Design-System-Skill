...
- 默认 DangoUI baseline 不做频次统计，也不要平铺全部 token；必须展示典型三级引用链：一级基础色板 -> 二级语义 token -> 三级组件别名。一级基础色板是 `purplegray-9` 这类无具体业务语义的色阶族，不是废弃的 `--du-c-*`。二级要说明如何引用/解析一级，三级必须展示如何通过 `var(--du-*)` 引用二级，不能只显示最终 16 进制值。
