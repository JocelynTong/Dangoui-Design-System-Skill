...
- 默认 DangoUI baseline 不做频次统计，也不要平铺全部 token；展示典型三级关系即可：一级基础色板、二级语义 token、三级组件别名。一级基础色板是 `purplegray-9` 这类无具体业务语义的色阶族，不是废弃的 `--du-c-*`；三级要展示 `var(--du-*)` 如何引用二级，不能只显示最终 16 进制值。
